...
void InterpolationBehaviour::InterpolationManager(float deltaTime) {
	...
	if (completedTransformsTotal < totalTforms) {
		if(currentTransform.hasFrames[0]) if (!currentTransform.transformFinished[0]) GetGameObject()->SetPostion(PerformTransformation(deltaTime, 0, currentTransform.animKeys[0]));
