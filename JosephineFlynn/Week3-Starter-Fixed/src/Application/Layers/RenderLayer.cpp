...
void RenderLayer::OnRender(const Framebuffer::Sptr& prevLayer)
{
	...
	app.CurrentScene()->Components().Each<RenderComponent>([&](const RenderComponent::Sptr& renderable) {
		...
		if (object->Has<MorphRenderComponent>()) {
			bool t = (app.CurrentScene()->FindObjectByName("Graphics_Manager")->Get<MorphInputManager>()->toggledOn);
			shader->SetUniform("t_val", object->Get<MorphRenderComponent>()->_t);
			shader->SetUniform("MorphOn", (int)t);
		}
